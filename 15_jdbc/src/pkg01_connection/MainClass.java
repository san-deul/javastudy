...
/*
 * oracle.jdbc.OracleDriver < 먼저 준비
 * 1. 오라클 데이터베이스에 접속하기 위해서 필요한 드라이버 클래스이다.
 * 2. 오라클에서 만들어서 배포한다.
 * 3. 18C XE 버전의 경우 : C:\app\사용자\product\18.0.0\dbhomeXE\jdbc\lib\ojdbc8.jar 파일을 프로젝트의 build path에 등록해야 한다.
 */
...
public class MainClass {
  ...
  public static void method1() {
    ...
    try {
      Class.forName("oracle.jdbc.OracleDriver");  // 전체이름 적어줘야함!!!
  ...
  public static void method2() {
    ...
    try {
      ...
      try {
        if(con != null) //<-조건. 해줘
        con.close();    // <-자체가 try catch구문을 요구해서 finally 문 안에서 또 try catch문을 넣어주는거임
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
  ...
  public static void method3() {
    ...
    Properties props = System.getProperties(); // 이미 등록되어있는 모든 프로퍼티 가져옴
    
    Enumeration enumeration = props.propertyNames();
    
    while(enumeration.hasMoreElements()) {//enumeration.의 데이터가 있으면
  ...
  public static void method4() {
    ...
    try {
      con = DriverManager.getConnection(url, user, password);
      System.out.println("데이터 베이스에 접속되었습니다.");
    } catch (Exception e) {
      e.printStackTrace();
    }finally {
      try {
