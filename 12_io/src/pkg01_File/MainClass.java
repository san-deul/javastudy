...
public class MainClass {
  ...
  public static void method3() {
    ...
    for(File file : files ) {
      ...
      // 이름 
      builder.append(String.format("%-20s", file.getName().substring(0,Math.min(file.getName().length(), 20)))); // 20자리 폭을 가진 이름임
      
      // 최종수정일
      String lastModified = new SimpleDateFormat("yyyy-MM-dd a h:mm").format(file.lastModified());
      builder.append(String
          .format("%-20s", lastModified));
      ...
      /*앞에건 몫을 구해주고 , 뒤에거는 몫을 구한 나머지...
       * 근데 어차피 1024가 1이 되니까 1024가 안되면 0인거임
       * 그래서 삼항연산자 써서 0 / 1 둘 중 하나를 선택하게 한거임ㄴ
       */
          //※ 1024 -> 1
      if(byteSize !=0) {
  ...
  public static void practice1() {
    ...
    // C:\2024\01\22 디렉터리 만들기(현재날짜의 디렉터리 만들기)
    //Date date = new Date();
  ...
  public static void practice2(File dir) {
    ...
    //재귀호출로 해결하기(자기가 자기를 호출하는 방식) 
    
    //practice(new File(\2024\01\22)) 호출후
    //practice(new File(\2024\01)) 호출후
    //practice(new File(\2024)) 호출
    //practice(new File(\)) 호출로 마무리
